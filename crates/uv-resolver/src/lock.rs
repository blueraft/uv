    ...
    fn to_toml(&self, table: &mut Table) {
        ...
        match *self {
            ...
            Source::Git(ref url, ref git_source) => {
                ...
                source_table.insert("revision", Value::from(git_source.precise.to_string()));
                ...
                let precise: GitSha =
                    revision
                        ...
                        .map_err(|err| LockErrorKind::InvalidGitSourceRevision {
                            given: revision.to_string(),
                            err,
                        })?;
                let kind = match (tag, branch) {
                    (None, None) => GitSourceKind::DefaultBranch,
                    (Some(tag), None) => GitSourceKind::Tag(tag),
                    (None, Some(branch)) => GitSourceKind::Branch(branch),
                    (Some(tag), Some(branch)) => {
                        let kind = LockErrorKind::InvalidGitSourceTagAndBranch { tag, branch };
...
enum LockErrorKind {
    ...
    /// Failed because a git source has both `tag` and `branch` fields.
    #[error(
        "failed to parse git source since it has both \
         tag={tag} and branch={branch}, must choose \
         one or the other"
    )]
    InvalidGitSourceTagAndBranch { tag: String, branch: String },
