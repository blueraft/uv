...
use uv_toolchain::{
    find_toolchains, DiscoveryError, EnvironmentPreference, Toolchain, ToolchainFetchStrategy,
...
pub(crate) async fn list(
    ...
    toolchain_fetch: ToolchainFetchStrategy,
    preview: PreviewMode,
    cache: &Cache,
    printer: Printer,
) -> Result<ExitStatus> {
    ...
    let download_request = match kinds {
        ...
        ToolchainListKinds::Default => {
            if toolchain_fetch.allows_fetch() {
                ...
                // If fetching is not allowed, then don't show downloads as available by default
                None
            }
        }
    };
